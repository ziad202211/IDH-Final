  ...
  return (
    <main className="overflow-x-hidden pt-16 bg-[#F8F8F8]">
        ...
        <section className="relative bg-white">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 mt-8">
            <div className="flex flex-wrap items-center gap-2 py-4 text-sm">
              <Link href="/projects" className="text-[#171A1F] hover:underline flex items-center">
              ...
              >
                <span className="text-[#171A1F]">{project.category}</span>
              </Link>
              <span className="text-gray-400 hidden sm:inline">/</span>
              <span className="text-[#171A1F] font-medium truncate">{project.title}</span>
            </div>
            ...
            <div className="relative h-[260px] sm:h-[360px] md:h-[490px] w-full mb-12 rounded-xl overflow-hidden shadow-md">
              <Image
              ...
              <div className="absolute inset-0 flex items-center justify-between px-2 sm:px-4">
                <Button
                  size="icon"
                  className="bg-black/40 hover:bg-black/70 rounded-full h-10 w-10 sm:h-12 sm:w-12"
                  onClick={prevImage}
                >
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <div className="space-y-8">
                <div className="border border-[#dee1e6] rounded-lg overflow-hidden text-sm sm:text-base">
                  {[
                    ...
                    <div key={i} className="grid grid-cols-2 border-b border-[#dee1e6] last:border-b-0">
                      <div className="p-3 sm:p-4 border-r border-[#dee1e6] font-medium text-[#171A1F]">
                        {label}
                      </div>
                      <div className="p-3 sm:p-4 text-[#171A1F]">{value}</div>
                    </div>
                  ))}
                </div>
                ...
                <Link href="/contact">
                  <Button className="btn-primary px-6 sm:px-8 py-4 sm:py-5 text-base sm:text-lg mt-4">
                    Contact us
                  </Button>
                </Link>
              </div>
      ...
      <SectionAnimation direction="right" delay={300}>
        <section className="bg-white py-16 sm:py-24" ref={scrollRef}>
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-[#171A1F] mb-10 sm:mb-12 text-center">
              Project Gallery
            </h2>
            ...
            <div
              className="
                ...
                gap-4 sm:gap-6 [column-fill:_balance]
              "
            >
              {project.images.map((img, idx) => (
                <div
                  ...
                  className="
                    mb-4 sm:mb-6 break-inside-avoid overflow-hidden
                    rounded-2xl sm:rounded-3xl relative group cursor-pointer
                  "
                ...
                >
                  <Image
                    ...
                    height={800}
                    className="
                      w-full h-auto object-cover rounded-2xl sm:rounded-3xl
                  ...
                  <div
                    ...
                    "
                  >
                    <p className="text-white text-xs sm:text-sm uppercase tracking-wide">
                      View
                    </p>
                  </div>
                </div>
      ...
      {/* Testimonials */}
      <SectionAnimation direction="up" delay={500}>
        <section className="py-16 sm:py-20 bg-gradient-to-b from-[#0F1115] to-[#1D2128]">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl border border-gray-800">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
                <div className="bg-black p-6 sm:p-10 flex flex-col justify-between">
                  <div>
                    <h2 className="text-2xl sm:text-3xl font-semibold text-[#636AE8] mb-3 sm:mb-4">
                      Testimonials
                    </h2>
                ...
                {testimonials.map((t, i) => (
                  <div
                    key={i}
                    className="p-6 sm:p-10 border-t md:border-t-0 border-r border-gray-800 hover:bg-[#22252E] transition-colors duration-300"
                  >
                    <p className="text-gray-300 text-sm sm:text-base leading-relaxed mb-6 sm:mb-8">
                      “{t.text}”
                    </p>
                    <div>
                      <h3 className="text-base sm:text-lg font-medium text-white">{t.name}</h3>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {isLightboxOpen && project && (
        <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center">
          <Button
            size="icon"
            className="absolute top-4 right-4 sm:top-6 sm:right-6 bg-white/20 hover:bg-white/40 rounded-full"
          ...
          <Button
            size="icon"
            className="absolute left-4 sm:left-6 bg-white/20 hover:bg-white/40 rounded-full"
            onClick={prevLightboxImage}
          >
            <ChevronLeft className="h-6 sm:h-7 w-6 sm:w-7 text-white" />
          </Button>

          <div className="relative w-[90vw] h-[60vh] sm:h-[80vh]">
            <Image
          ...
          <Button
            size="icon"
            className="absolute right-4 sm:right-6 bg-white/20 hover:bg-white/40 rounded-full"
            onClick={nextLightboxImage}
          >
            <ChevronRight className="h-6 sm:h-7 w-6 sm:w-7 text-white" />
          </Button>
        </div>
      )}
    </main>
